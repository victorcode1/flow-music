...
/// Reproduce una emisora resolviendo su stream. Las pantallas de radio
/// comparten este flujo para evitar duplicar logica.
///
...
Future<void> playRadioStation({
  ...
  await _resolveAndPlay(context: context, station: station);
}
...
Future<void> playNextRadioStation({
  ...
  if (next == null) return;
  await _resolveAndPlay(context: context, station: next);
}
...
Future<void> playPreviousRadioStation({
  ...
  await _resolveAndPlay(context: context, station: previous);
}
